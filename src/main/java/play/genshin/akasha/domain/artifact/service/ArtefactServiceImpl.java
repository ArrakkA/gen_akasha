...
public class ArtefactServiceImpl implements ArtefactService{
    ...
    @Override
    public List<ArtifactDTO> makeArtefactScore(String charName, String partyType) {
    ...
    public void resisterArtefact(ArtifactRequestDTO dto){
        int artifactNo = artifactRepository.findArtifactNo(dto.getArtifactCd(), dto.getUserName());

        Artifact artifact = new Artifact(dto, artifactNo);
